...
} from "./platform/android/index.js";
import {
  updateIOSPlist,
...
async function removeLegacyAssociatedDomainFiles(
  androidManifestUri?: vscode.Uri,
  iosPlistUri?: vscode.Uri,
): Promise<void> {
  // Remove only the exact legacy files when their JSON matches the shape
  // generated by older releases. Parent directories and unrelated assets are
  // deliberately left untouched.
  const legacyFiles = [
    androidManifestUri
      ? vscode.Uri.file(path.join(path.dirname(androidManifestUri.fsPath), "assets", ".well-known", "assetlinks.json"))
      : undefined,
    iosPlistUri
      ? vscode.Uri.file(path.join(path.dirname(iosPlistUri.fsPath), "Assets.xcassets", ".well-known", "apple-app-site-association"))
      : undefined,
  ].filter((uri): uri is vscode.Uri => !!uri);
  for (const legacyUri of legacyFiles) {
    try {
      const parsed = JSON.parse(Buffer.from(await vscode.workspace.fs.readFile(legacyUri)).toString("utf8")) as unknown;
      const legacyAasa = parsed as {
        applinks?: { apps?: unknown[]; details?: Array<{ appID?: unknown; paths?: unknown[] }> };
      };
      const generatedShape = Array.isArray(parsed)
        ? parsed.length === 1
          && parsed[0]?.relation?.length === 1
          && parsed[0].relation[0] === "delegate_permission/common.handle_all_urls"
          && parsed[0]?.target?.namespace === "android_app"
          && typeof parsed[0]?.target?.package_name === "string"
          && Array.isArray(parsed[0]?.target?.sha256_cert_fingerprints)
        : !!(
          parsed
          && typeof parsed === "object"
          && Object.keys(parsed).length === 1
          && Array.isArray(legacyAasa.applinks?.apps)
          && legacyAasa.applinks.apps.length === 0
          && Array.isArray(legacyAasa.applinks?.details)
          && legacyAasa.applinks.details.length === 1
          && typeof legacyAasa.applinks.details[0]?.appID === "string"
          && legacyAasa.applinks.details[0]?.paths?.length === 1
          && legacyAasa.applinks.details[0].paths[0] === "*"
        );
      if (generatedShape) {await vscode.workspace.fs.delete(legacyUri);}
    } catch {
      // Missing, user-authored, or invalid legacy files are not touched.
    }
  }
}
...
async function updateAssociatedDomainFiles(
  ...
  const domains = service.values?.domains
    ?.split(/[,;\n]+/)
    .map((domain) => domain.trim().replace(/^applinks:/i, "").replace(/^https?:\/\//i, "").split("/")[0])
  ...
  const outputRoot = vscode.Uri.joinPath(workspaceFolder.uri, ".flutter-config-manager", "app-links-hosting");

  try {
    await removeLegacyAssociatedDomainFiles(androidManifestUri, iosPlistUri);

    // This directory is entirely generated and is rebuilt so removed domains
    // cannot leave stale verification files behind.
    try {
      await vscode.workspace.fs.delete(outputRoot, { recursive: true });
    } catch {
      // It is normal for the generated directory not to exist yet.
    }

    for (const domain of Array.from(new Set(domains))) {
      const wellKnownDir = vscode.Uri.joinPath(outputRoot, domain, ".well-known");
      ...
      if (androidManifestUri && packageName && fingerprints.length > 0) {
        ...
        await vscode.workspace.fs.writeFile(
          vscode.Uri.joinPath(wellKnownDir, "assetlinks.json"),
      ...
      if (iosPlistUri && teamId && bundleId) {
        ...
        await vscode.workspace.fs.writeFile(
          vscode.Uri.joinPath(wellKnownDir, "apple-app-site-association"),
    ...
    const instructions = [
      ...
      "Deploy each domain directory's .well-known contents to the matching HTTPS domain without redirects.",
      "These files are hosting artifacts and must not be bundled into the mobile application.",
      "",
    ].join("\n");
    await vscode.workspace.fs.writeFile(
      vscode.Uri.joinPath(outputRoot, "README.txt"),
      new TextEncoder().encode(instructions),
    );
    return vscode.workspace.asRelativePath(outputRoot, false);
  } catch (e) {
    logger.error("Failed to generate App Links hosting files:", toError(e));
    throw e;
  }
}

async function removeGeneratedAssociatedDomainFiles(
  androidManifestUri?: vscode.Uri,
  iosPlistUri?: vscode.Uri,
): Promise<void> {
  await removeLegacyAssociatedDomainFiles(androidManifestUri, iosPlistUri);
  ...
  const outputRoot = vscode.Uri.joinPath(
    workspaceFolder.uri,
  ...
  try {
    ...
    // Missing generated output is already the desired state.
  }
}

async function updateGeneratedDartServiceConfig(
  services: ServiceEntry[],
  servicesConfig: ServiceConfig[],
): Promise<void> {
  const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
  if (!workspaceFolder) {return;}
  const uri = vscode.Uri.joinPath(workspaceFolder.uri, ...GENERATED_SERVICES_DART_PATH.split('/'));
  const content = buildDartServiceConfig(services, servicesConfig);
  if (content) {
  ...
  try {
    ...
    if (existing.startsWith('// Generated by Flutter Config Manager.')) {
...
export async function savePermissions(ctx: SaveContext): Promise<SaveResult> {
  ...
  try {
    ...
    if (servicesConfig) {
      await updateGeneratedDartServiceConfig(services || [], servicesConfig);
    }
    ...
    } else if (removedServiceIds.includes(APPLINKS_SERVICE_ID)) {
      await removeGeneratedAssociatedDomainFiles(androidManifestUri, iosPlistUri);
    }
...
export async function saveServicesOnly(
  ...
  try {
    ...
    if (iosPodfileUri) {
      try {
        const podfileDoc = await vscode.workspace.openTextDocument(iosPodfileUri);
        const current = podfileDoc.getText();
        const updated = updateIOSPodfileWithServices(current, services || [], servicesConfig);
        if (updated !== current) {
          await replaceDocumentContent(iosPodfileUri, updated);
        }
      } catch (podError) {
        logger.error("Podfile service update error:", toError(podError));
      }
    }
    ...
    let appLinksOutput: string | undefined;
    if (applinksService) {
      appLinksOutput = await updateAssociatedDomainFiles(
        applinksService,
        androidManifestUri,
        iosPlistUri,
      );
    ...
    return {
      success: true,
      message: appLinksOutput
        ? `Services saved. Deploy App Links verification files from ${appLinksOutput}.`
