...
import { applyTabVisibility } from '../../core/router.js';
...
export function renderAndroidTable() {
  ...
  sorted.forEach((permission, index) => {
    ...
    // Actions: equivalent + delete buttons
    ...
    if (
      ...
      rowActions.appendChild(equivalentButton);
    }
...
/**
 * Auto-resizes a value <textarea> to fit its content so long usage
 * descriptions are readable without manual resizing.
 */
...
export function renderModalCategoryTabs() {
  ...
  // macOS uses the same NS* permission catalog as iOS.
  const isApplePlatform =
    state.modalMode === "ios" || state.modalMode === "macos";
...
export function updateCounts() {
  ...
  const macosCount = state.macosPermissions
    ...
    : 0;
  if (androidCountChip) { androidCountChip.textContent = `Android: ${androidCount}`; }
        ...
        if (!existing) {
          ...
          if (iosPermission) {
            ...
            if (type === "boolean") {
              ...
              if (!value || value === "TODO: Provide usage description.") {
                if (valueInput) { valueInput.style.borderColor = "var(--danger)"; }
