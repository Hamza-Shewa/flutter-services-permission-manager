...
import * as permissions from './features/permissions/permissions.js';
...
bus.on("saveResult", (message) => {
  ...
  // On failure, make sure any validator/asset spinner is dismissed so the UI
  // never stays stuck on "Installing..." / "Analyzing...".
  if (!message.success) {
...
bus.on("packagesAnalysisResult", (message) => {
  ...
  if (message.error) {
    if (message.error.includes("the current project is not a flutter project") ||
      message.error.includes("requires VPN access") ||
      message.error.includes("git-hosted dependency is unreachable")) {
