...
const SERVICES_MARKER_START = '# --- BEGIN FLUTTER CONFIG SERVICES ---';
const SERVICES_MARKER_END = '# --- END FLUTTER CONFIG SERVICES ---';
...
${macroEntries}
    ...
    // Derive deployment target from the Podfile's existing platform line, or fall back to 13.0
    const platformMatch = content.match(/platform\s*:ios,\s*['"]([^'"]+)['"]/i);
    const deploymentTarget = platformMatch?.[1] ?? "13.0";
...
/**
 * Update Podfile with service-specific targets
 */
export function updateIOSPodfileWithServices(
    content: string,
    services: ServiceEntry[],
    servicesConfig: ServiceConfig[]
): string {
    let updated = content;
    const blockRegex = new RegExp(`\\n*${SERVICES_MARKER_START}[\\s\\S]*?${SERVICES_MARKER_END}\\n*`, 'g');
    updated = updated.replace(blockRegex, '\n');

    const podfileTargets: string[] = [];
    
    for (const service of services) {
        const config = servicesConfig.find(c => c.id === service.id);
        if (config?.ios?.podfileTargets) {
            for (const target of config.ios.podfileTargets) {
                podfileTargets.push(`target '${target.targetName}' do\n${target.code}\nend`);
            }
        }
    }
    ...
    if (podfileTargets.length > 0) {
        ...
        updated += `\n\n${SERVICES_MARKER_START}\n# Generated by Flutter Config Manager. Do not modify manually.\n`;
        updated += podfileTargets.join('\n\n');
        updated += `\n${SERVICES_MARKER_END}\n`;
    }
